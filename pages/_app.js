...
function FCMProvider() {
  ...
  // Don't show banner if: no user, already decided, or dismissed this session
  const showBanner = !!user && permissionState === "default" && !bannerDismissed;

  const handleEnable = async () => {
    const result = await requestPermission();
    if (result === "granted") setBannerDismissed(true);
    else setBannerDismissed(true); // hide either way after they've decided
  };

  return (
    <>
      {/* ── Notification enable banner (shown once, needs tap to satisfy user-gesture) ── */}
      {showBanner && (
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              color: "#0a0a0a", cursor: "pointer", fontFamily: "'Lexend', sans-serif",
              flexShrink: 0,
            }}
          >
            Enable
