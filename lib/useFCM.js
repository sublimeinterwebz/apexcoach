  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // forceRefresh: true — always get a fresh token tied to the current VAPID key
        // Fixes stale tokens captured before the env var was correctly set
        const token = await getToken(messaging, { vapidKey, serviceWorkerRegistration: swReg, forceRefresh: true });
        if (token) {
