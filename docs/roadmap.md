...
| Item                                                        | Commit     |
|-------------------------------------------------------------|------------|
| Onboarding & Gemini Structured Outputs: added "Training Modality / Style" and "Max Session Duration" to onboarding and profile edit flows. Updated `/api/generate-plan` to categorize exercises internally as compounds vs isolations via keyword inference. Decoupled prompt into `systemInstruction` vs user `prompt`. Implemented strict `responseSchema` mapped to the precise JSON schema required for Vercel outputs — Gemini API natively validates output format and mathematically prevents JSON-breakage or text-outside-json issues. | `5120f63` |
